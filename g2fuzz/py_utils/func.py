...
# -------------------- Improved system message and prompts --------------------
...
messages_feature = [
    {"role": "system", "content": "You are an expert in file structures and file formats; provide precise, structured feature lists when prompted."},
    {"role": "user", "content": ""}
]
...
<TARGET_FEATURES>
```
Output only a single Markdown code block containing the runnable Python script that writes examples to `./tmp/`. Do not include prose or additional code blocks. If external libraries are required, state them in a short top-of-file comment and provide a pure-Python fallback.
"""
...
# Feature prompts remain the same format but clarified earlier
...
# Improved generator mutation prompt
...
Goal
----
Extend the given generator `<TARGET_GENERATOR>` so it produces `<FROMAT>` files that include an additional feature `<NEW_FEATURE>` described below. The output must be a runnable generator (Python) that writes examples to `./tmp/`.
...
Requirements
- Keep the same file format/family and preserve original behavior when possible.
- Add clear header comments describing the new feature and any small usage/demo.
- Output must be a single Markdown code block containing only the final Python code (no extra prose).
